...
def test_prob_word_given_class(target, word_frequency, class_frequency):
 ...
 {'name': 'test_case_19',
  ...
  'expected': 0.04039497307001795}
    ...
    for test_case in test_cases:
        output = target(word_frequency, class_frequency, **test_case['input'])
        expected = test_case['expected']
        ...
        if not np.isclose(output, expected):
            ...
            print(f"Wrong value for P({test_case['input']['word']} | {test_case['input']['cls']}) in {failed_cases[-1]['name']}. Expected: {failed_cases[-1]['expected']}. Got: {failed_cases[-1]['got']}")
            continue
        successful_cases += 1
